...
socket_t hostname_connect(const std::string& hostname, int port) {
    ...
    for(p = result; p != NULL; p = p->ai_next)
    {
        ...
        {
            printf("%02x ", int(p->ai_addr->sa_data[ii]));
        }
        printf("\nsocket(): %s %d %d %d\n", hostname.c_str(), p->ai_family, p->ai_socktype, p->ai_protocol);
        ...
        else
        {
            if((errno == EINPROGRESS) || (errno == EWOULDBLOCK) || (errno == EISCONN))
            {
                ...
                while(select(sockfd + 1, nullptr, &sockets, nullptr, nullptr) <= 0) { printf("select %d\n", errno); }
